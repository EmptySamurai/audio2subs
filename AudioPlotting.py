...
def index_to_hz(index, length, sampleRate):
    return index * sampleRate / length


def hz_to_index(hz, length, sampleRate):
...
def voice_frequency_energy(frame, sampleRate):
    length = len(frame)
    start_index = hz_to_index(300, length, sampleRate)
    end_index = hz_to_index(1000, length, sampleRate)
    sum_energy = 0
...
(sampleRate, samples) = read(pathToFile)
samples = np.array(samples, dtype="float") / (2 << 16)

frameLength = 10  #ms
frameSamples = int((sampleRate * frameLength) / 1000)
number_of_frames = len(samples) // frameSamples
...
dominant_frequencies = [most_dominant_frequency(frame, sampleRate) for frame in ffts]
frequency_energy = [voice_frequency_energy(frame, sampleRate) for frame in ffts]
...
cur = 0
axarr[cur].plot(np.arange(len(samples)) / sampleRate, samples, 'b')
axarr[cur].set_title("Sound")
cur += 1
axarr[cur].plot(np.arange(len(energy)) * frameLength / 1000, energy, 'r')
axarr[cur].set_title("Short-time energy")
cur += 1
axarr[cur].plot(np.arange(len(dominant_frequencies)) * frameLength / 1000, dominant_frequencies, 'g')
axarr[cur].set_title("Most dominant frequencies")
cur += 1
axarr[cur].plot(np.arange(len(frequency_energy)) * frameLength / 1000, frequency_energy, '#00FCF8')
axarr[cur].set_title("Frequency energy")
cur += 1
axarr[cur].plot(np.arange(len(spectral_flatness_frames)) * frameLength / 1000, spectral_flatness_frames, '#92C610')
axarr[cur].set_title("Spectral flatness")
cur += 1
axarr[cur].plot(np.arange(len(speech_energy_detection)) * frameLength / 1000, speech_energy_detection, '#FFA305')
axarr[cur].set_title("Speech detection based on energy")
cur += 1
axarr[cur].plot(np.arange(len(speech_frequency_energy_detection)) * frameLength / 1000,
                speech_frequency_energy_detection,
                '#C905FF')
axarr[cur].set_title("Speech detection based on frequency energy")
cur += 1
axarr[cur].plot(np.arange(len(speech_both_energy_detection)) * frameLength / 1000, speech_both_energy_detection,
                '#DB7093')
